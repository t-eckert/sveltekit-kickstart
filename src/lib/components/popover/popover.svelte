		...
		>
			<div class="flex items-center">
				...
				<div class="flex flex-col">
					<h4 class="text-[17px] leading-5 font-semibold tracking-[-0.01em]">Resize image</h4>
					<p class="text-muted-foreground text-sm font-medium">Resize your photos easily</p>
				</div>
			</div>
			<Separator.Root class="bg-dark-10 -mx-4 mt-[17px] mb-6 block h-px" />
			<div class="flex items-center pb-2">
				<div class="mr-2 flex items-center">
					<div class="relative mr-2">
						<span class="sr-only">Width</span>
						<span aria-hidden="true" class="text-xxs text-muted-foreground absolute top-4 left-5"
							>W</span
						>
						<input
							type="number"
							class="h-input rounded-10px border-border-input bg-background text-foreground w-[119px] border pr-2 pl-10 text-base sm:text-sm"
							bind:value={width}
						/>
					</div>
					<div class="relative">
